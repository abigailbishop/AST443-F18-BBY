...
pwd = '/astrolab/Fall_18/abishop/AST443-F18-BBY/Lab0/'
fname1 = pwd + '3.3_SpectrumFlats_all/master_DomeFlat_50E-6m.fits'
fname2 = pwd + '3.3_SpectrumFlat/masterflat_lamp.fits'

# Open files
masterflat_f = fits.open(fname1)
masterflat = masterflat_f[0].data
masterflat_np = np.asarray(masterflat)
masterflat_shape =  masterflat.shape
lampflat_f = fits.open(fname2)
...
## Find length of spectrum ##

# slope from spectral_lineup_plot.py
m = 0.98 # A/pix
shift=0

# convert length of spectrum to angstrums
print 'length of spectrum =',len(x)*m
  # 749.7

## Convert Spectrum ##
x_spec = x*m + shift
y_spec = ylamp_norm
np.savetxt('calibrated_arcspec.txt',np.c_[x_spec,y_spec])
